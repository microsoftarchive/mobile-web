...
namespace MileageStats.Data.InMemory
{
    public class UserRepository : InMemoryRepository<User>, IUserRepository
    {
        ...
        /// <returns>The <see cref="User"/> populated with a shallow version of a Vehicle and the Vehicle's photo.</returns>
        public User GetByAuthenticatedId(string claimedIdentifier)
        {
            return Set.FirstOrDefault(u => u.AuthorizationId == claimedIdentifier);
        }
    }
}
